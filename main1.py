...
# CAMERA AS INPUT WITH LINE AND TWILIO SENDER
# This code takes real time camera as input and sends alert via TWILIO

# python main1.py --prototxt mobilenet_ssd/MobileNetSSD_deploy.prototxt --model mobilenet_ssd/MobileNetSSD_deploy.caffemodel
...
def run():
    ...
    if config.Thread:
        vs = thread.ThreadingClass(src=0)
    ...
    while True:
        ...
        # if the frame is None, we have reached the end of the video
        if frame is None:
            break
